...
前三行依次输出顶部横线、表头和分隔线；表头中的列名及其间距都直接写入字符串，避免格式化时改变空格数量。

### 第 4~8 行：数据区

五条省份数据同样使用固定字符串输出。面积和人口字段已经按样例保留两位小数，省名后的空格也必须原样保留。

### 第 9 行：底部横线

最后再次输出与顶部相同的横线，表格至此正好完成 9 行输出。
...
```text
输出与“输出样例”中的 9 行完全相同，首尾横线均为 36 个连字符。
```
...
```text
表头、5 条省份数据和底部横线的空格位置必须与输出样例一致，输出内容不随输入改变。
```

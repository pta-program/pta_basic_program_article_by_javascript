...
```
代码先把输入厘米数解析为整数，再按 2.54 厘米/英寸换算为总英寸，并用 `Math.trunc` 取完整英寸数。

### 换算总英寸

`totalInch = Math.trunc(cm / 2.54 + 1e-9)`，加上极小修正量是为了避免浮点表示误差使本应为整数的结果略小于整数。

### 换算英尺和剩余英寸

总英寸除以 12 的整数商是英尺，余数是英寸，因此 `foot = Math.trunc(totalInch / 12)`、`inch = totalInch % 12`。

### 输出结果

使用模板字符串按 `英尺 英寸` 的顺序输出，数值结果不会额外带小数部分。
